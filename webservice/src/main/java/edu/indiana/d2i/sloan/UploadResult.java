...
public class UploadResult {
  ...
  public Response getResourcePost(
    ...
    try {
      ...
      if (!Configuration.getInstance().getBoolean("result.review.human", false)) {
        // add to post-process
        UploadPostprocess.instance.addPostprocessingItem(new UserResultBean(
            userbean.getUserName(), userbean.getUserEmail(), randomid));
      } else {
        String emails = Configuration.getInstance().getString("result.review.email");
        if (emails == null) {
          logger.error("Reviewers\' email addresses are missing.");
        } else {
          String[] reviewAddresses = emails.split(";");

          for (String email : reviewAddresses) {
            EmailUtil emailUtil = new EmailUtil();
            emailUtil.sendEMail(email, "HTRC Data Capsules Result Review Request", "Dear Reviewer, \n" +
