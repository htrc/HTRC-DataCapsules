...
public class VMStateManager {
	...
	public synchronized boolean transitTo(String userName, String vmid,
			VMState target) throws SQLException, NoItemIsFoundInDBException {
		// get current state from db
		List<VmStatusBean> status = DBOperations.getInstance().getVmStatus(
				userName, vmid);
		...
		if (canTrasist) {
			DBOperations.getInstance().updateVMStatus(vmid, target);
			logger.info("Transit from " + current + " to " + target
					+ " for vm " + vmid);
			return true;
		} else {
			logger.error("Cannot transit from " + current + " to " + target
					+ " for vm " + vmid);
			return false;
		}
	}
